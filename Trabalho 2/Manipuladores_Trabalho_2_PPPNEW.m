...
% Cirando um Gif
nome_gif = 'Manipuladores_Trabalho_2_PPP.gif';
grafico = figure;
...
F1 = transformation(F0, 3, 90, 0, -90);

% Transformação do frame {1} para o frame {2}:
F2 = transformation(F1, l1, -90, 0, -90);

% Transformação do frame {2} para o frame {3}:
F3 = transformation(F2, l2, 90, 0, -90);

% Transformação do frame {3} para o frame {4}:
F4 = transformation(F3, l3, 0, 0, 0);
...
for theta = 0:30
    ...
    % Parâmetros Gráficos
    plot3(0,0,0);   %title(['Translação: ' num2str(link)]);
    ...
    f4 = double(subs(F4, [t1 t2 t3 t4 l1 l2 l3 l4],[T1 T2 T3 T4 L1 L2 L3 L4]));
